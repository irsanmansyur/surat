                             ...
                             <tbody>
                                 ...
                                 <?php foreach ($berkas as $b) : ?>
                                     <tr>
                                         ...
                                         <td><?= $b['pesan']; ?></td>
                                          <td><button class="tombol-isi-berkas btn btn-default btn-block btn-xs" data-id="<?= $b['id_berkas']; ?>" onclick="isi(<?= $b['id_berkas']; ?>)">Isi Berkas</button></td>
                                        <td><?php if ($b['status_berkas']==1) {
                                            echo "Belum dikirim";
                                        }else{

                                            echo "<span class='bg-success'>Sudah dikirim</span>";
                                        } ?></td>

                                         <?php if ($b['status_berkas'] == 1) : ?>
                                             <td><button class="tombol-edit btn btn-info btn-block btn-xs" onclick="edit_berkas(<?= $b['id_berkas']; ?>)" >Edit</button></td>
                                         <?php else : ?>
                                             <td>Closed</td>
                                         <?php endif; ?>
                                         <?php if ($b['status_berkas'] == 1) : ?>
                                         ...
                                         <?php else : ?>
                                             <td>Closed</td>
                                         <?php endif; ?>
                                         <td><a href="<?php echo base_url('user/cetak_surat/'.$b['id_berkas'].'/'.$b['id_template'].'') ?>" target="blank" class="btn btn-primary btn-xs">Cetak</a></td>
     ...
     <div class="modal-dialog">
         <div class="modal-content">
             ...
             <div class="modal-body">
                 ...
                 <div class="form-group">
                     <label>Pesan</label>
                     <textarea class="form-control ckeditor"  placeholder="Isi Pesan" name="pesan" id="pesan" ></textarea>
                 </div>
                 <div class="form-group">
 ...
 <script>
        ...
        var id_berkas=id;
         $.ajax({
             ...
             success: function(data) {
                document.getElementById('kd').value=data.kd_berkas;
                document.getElementById('nama').value=data.nama_berkas;
                document.getElementById('tuj').value=data.tuj_berkas;
                document.getElementById('tgl').value=data.tgl_berkas;
               // document.getElementById('pesan').insertHtml=data.pesan;
               CKEDITOR.instances.pesan.setData(data.pesan);
                document.getElementById('idberkas').value=data.id_berkas;
    ...
    function isi(id_berkas){

         var id_berkas = id_berkas;
        // alert(id_berkas);
         $.ajax({
             url: '<?php echo base_url('user/get_berkas'); ?>',
             data: {
             ...
             success: function(data) {
                $('#isi-surat').modal();
                document.getElementById('fileupload').value=data.file_upload;
                document.getElementById('idisiberkas').value=data.id_berkas;
     ...
     $('.tombol-kirim').on('click', function() {
         ...
         $.ajax({
             ...
             success: function(data) {
                 ...
                 $('#id_divisi').val(data.tuj_berkas);
             }
         });
     });
 </script>
 <script type="text/javascript">
     function template_surat(id_berkas){
//alert(id_berkas);
